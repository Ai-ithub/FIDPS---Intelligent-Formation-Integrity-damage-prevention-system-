...
class LWD_MWD_DataGenerator:
    ...
    def generate_dataset(self, start_date, duration_days):
        ...
        # Filter the DataFrame for rows marked as REAL_TIME
        ...
        topic = os.environ.get("KAFKA_TOPIC", "real-time-LWD_MWD-topic")

        # Initialize Kafka producer with broker address and API version
        producer = KafkaProducer(bootstrap_servers=['kafka:9092'],api_version=(2,1))

        while True:  
            # Save filtered data to CSV for logging or backup          
            df[real_time_mask].to_csv(f"real_time_data.csv", index=False)

            # Iterate over filtered rows and send each as a Kafka message
            for row in df[real_time_mask]:
                message = str(row).encode('utf-8') # Convert row to dict and encode
                producer.send(topic, message)  # Send message to Kafka topic
                print(f"Sent: {message}")  # Log sent message
            
                print("Data generation complete!")
                
        # Return the DataFrame (though unreachable due to infinite loop)
        return df
...
if __name__ == "__main__":
    ...
    duration_days = 1  # 6 months
    output_path = "formation_damage_dataset"
    
    df = generator.generate_dataset(start_date, duration_days)
