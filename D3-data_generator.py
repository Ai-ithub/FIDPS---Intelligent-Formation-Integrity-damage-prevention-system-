...
json.JSONEncoder.default = lambda self, obj: (obj.isoformat() if isinstance(obj, datetime)else obj.value if isinstance(obj, Enum)
    else None)
...
class LWD_MWD_DataGenerator:
    def __init__(self):
        ...
        self.damage_probabilities = {DamageType.FLUID_LOSS: 1}
    ...
    def generate_formation_data(self, depth):
        """Generate realistic formation-related metadata"""
        if depth < 500:
            formation_type = "UNCONSOLIDATED_SAND"
            lithology = "SANDSTONE"
        elif depth < 1500:
    ...
    def simulate_damage_effects(self, current_values, damage_type, severity):
        ...
        if damage_type == DamageType.FLUID_LOSS:
            # Fluid loss effects
    ...
    def generate_damage_event(self, current_depth, current_time):
        ...
        if random.random() < 0.001:  # 0.1% chance of damage event per record
            damage_type = DamageType.FLUID_LOSS
    ...
    def generate_drilling_phase(self, current_depth):
        """Determine the drilling phase based on depth"""
        if current_depth < 100:
            return "SPUDDING"
        elif current_depth < 1000:
            return "SURFACE_HOLE_DRILLING"
        elif current_depth < 3000:
            return "INTERMEDIATE_HOLE_DRILLING"
        elif current_depth < 4500:
            return "PRODUCTION_HOLE_DRILLING"
        else:
             return "LOGGING_AND_COMPLETION"
    ...
    def generate_dataset(self, start_date, duration_days, output_path):
        ...
        for i, timestamp in enumerate(timestamps):
            ...
            if active_damage_event and timestamp <= active_damage_event['end_time']:
                ...
                damage_type_obj = next(dt for dt in DamageType if dt.value == active_damage_event['damage_type'])
                current_values = self.simulate_damage_effects(current_values, damage_type_obj, severity)
    ...
    def save_data_by_tier(self, df, output_path):
        ...
        raw_sensor_data = raw_sensor_data.copy()
        raw_sensor_data.loc[:, 'date'] = raw_sensor_data['timestamp'].dt.date
        raw_sensor_data = raw_sensor_data.loc[:, ~raw_sensor_data.columns.duplicated()]
        raw_sensor_data.columns = raw_sensor_data.columns.map(str)
...
if __name__ == "__main__":
    ...
    duration_days = 90  # 6 months

    output_path = "formation_damage_dataset-90days"
